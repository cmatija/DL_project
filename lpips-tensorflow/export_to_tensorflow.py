...
def main():
    ...
    print("using " + device)
    model = dm.DistModel()
    model.initialize(model=args.model, net=args.net, use_gpu=use_gpu, version=args.version)
    print('Model [%s] initialized' % model.name())
    if use_gpu:
        dummy_im0 = torch.Tensor(1, 3, args.image_height, args.image_width).cuda()  # image should be RGB, normalized to [-1, 1]
        dummy_im1 = torch.Tensor(1, 3, args.image_height, args.image_width).cuda()
    else:
    ...
    cache_dir = os.path.expanduser('~/.lpips/'+device)
    ...
    print('now doing pb stuff')
    from onnx_tf.backend import prepare
    tf_rep = prepare(model, device=device)
